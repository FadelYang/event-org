            ...
            @foreach ($eventParticipants as $eventParticipant)
                <tr class="">
                    ...
                    @if ($eventParticipant->status == 'success')
                        <td><span
                                class="px-2 py-1 leading-tight text-lg font-semibold  text-green-700 bg-green-100 rounded-full">{{ $eventParticipant->status }}</span>
                        </td>
                    @elseif ($eventParticipant->status == 'pending')
                        <td><span
                                class="px-2 py-1 leading-tight text-lg font-semibold text-orange-700 bg-orange-100 rounded-full">{{ $eventParticipant->status }}</span>
                        </td>
                    @else
                        <td><span
                                class="px-2 py-1 leading-tight text-lg font-semibold  text-red-700 bg-red-100 rounded-full">{{ $eventParticipant->status }}</span>
                        </td>
                    @endif
                </tr>
            @endforeach
